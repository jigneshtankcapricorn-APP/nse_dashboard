...
# Clean version — verified syntax, no legacy imports
# ============================================================
...
if not st.session_state.app_authed:
    ...
    with center:
        ...
        st.markdown("<div class='login-badge'>🔒 Secured · NSE Index Dashboard</div>", unsafe_allow_html=True)
        if auth_clicked:
            try:
...
if not st.session_state.logged_in:
    ...
    with center:
        ...
        """, unsafe_allow_html=True)
        st.write("")
        connect_clicked = st.button("🔐  Connect to Angel One", use_container_width=True, type="primary")
        st.markdown("<div class='login-badge'>🔒 Credentials loaded from Streamlit Secrets</div>", unsafe_allow_html=True)
        if connect_clicked:
            try:
...
def color_cell(val):
    if pd.isna(val): return "background-color:#2a2a3e;color:#666"
    if   val >  10:  bg = "#004400"
...
def color_cumulative(val):
    if pd.isna(val): return "background-color:#2a2a3e;color:#666"
    if   val >  30: bg = "#003300"
    elif val >  15: bg = "#005500"
    elif val >   0: bg = "#007700"
    elif val > -15: bg = "#aa0000"
    else:            bg = "#660000"
    return f"background-color:{bg};color:white;font-weight:800;font-size:13px"
...
def render_heatmap(matrix: pd.DataFrame):
    ...
    has_cum   = "52W Return%" in matrix.columns

    def style_col(col):
        if col.name == "52W Return%":
            return [color_cumulative(v) for v in col]
        if col.name in week_cols:
...
def render_index_chart(df: pd.DataFrame, index_name: str, chart_type: str):
    ...
    if chart_type == "Bar Chart":
        fig = go.Figure(go.Bar(
            x=weeks_label, y=pcts, marker_color=colors,
            text=[f"{p:+.2f}%" for p in pcts], textposition="outside",
        ))
    else:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=weeks_label, y=pcts, mode="lines+markers",
...
# ── Main Header ───────────────────────────────────────────────
st.title("📊 NSE Index Tracker — 32 Indices × 52 Weeks")
st.caption("Phase 1: Heatmap  |  Phase 2: Best/Worst  |  Phase 3: Trading Signals")

if not all_data:
    st.info("👈 Click **Refresh Data** in the sidebar to fetch all indices.")
    st.stop()
...
tab_broad, tab_sector, tab_thematic, tab_signals = st.tabs([
    "📈 Broad Market", "🏭 Sectoral", "🎯 Thematic", "🚦 Trading Signals",
])
            ...
            def style_bw(col):
                if col.name in ["Best %", "Worst %"]:
                    return ["color:#00cc66;font-weight:bold" if isinstance(v, float) and v > 0
                            ...
                            for v in col]
                return ["" for _ in col]
            st.dataframe(bw.style.apply(style_bw, axis=0), use_container_width=True, height=300)
...
# ── Trading Signals Tab ───────────────────────────────────────
with tab_signals:
    st.subheader("🚦 Trading Signals — All Indices")
    st.caption("Computed from weekly % change data.")
    ...
    with sig1:
        st.markdown("**Momentum** — Avg last 4W vs 13W.")
        mom = compute_momentum_signal(full_matrix)
        if not mom.empty:
            color_map = {"STRONG":"#00ff88","RISING":"#88ff44","FADING":"#ffaa00","WEAK":"#ff4444"}
            def style_mom(col):
                if col.name == "Momentum":
                    return [f"color:{color_map.get(v,'')};font-weight:bold" for v in col]
                return ["" for _ in col]
            st.dataframe(mom.style.apply(style_mom, axis=0), use_container_width=True, height=600)

    with sig2:
        st.markdown("**Rotation** — Last 4W vs prior 4W.")
        rot = compute_sector_rotation(full_matrix)
        if not rot.empty:
            def style_rot(col):
                if col.name == "Flow_Delta":
                    return ["color:#00cc66" if isinstance(v, float) and v > 0
                            else ("color:#ff4444" if isinstance(v, float) and v < 0 else "")
                            for v in col]
                return ["" for _ in col]
            st.dataframe(rot.style.apply(style_rot, axis=0), use_container_width=True, height=600)

    with sig3:
        st.markdown("**Breakout** — Distance from 52W high.")
        bo = compute_breakout_signal(all_data, all_cat_indices)
        if not bo.empty:
            signal_colors = {"BREAKOUT":"color:#00ff88;font-weight:bold","Near High":"color:#88ff44",
                             "Deep Correction":"color:#ff4444;font-weight:bold","Neutral":"color:#aaaaaa"}
            def style_bo(col):
                if col.name == "Signal":
                    return [signal_colors.get(str(v), "color:#aaaaaa") for v in col]
                return ["" for _ in col]
            st.dataframe(bo.style.apply(style_bo, axis=0), use_container_width=True, height=600)

    with sig4:
        st.markdown("**Weakness** — Consecutive red weeks.")
        weak = compute_weakness_signal(full_matrix)
        if not weak.empty:
            weak_colors = {"HIGH RISK":"color:#ff2222;font-weight:bold","WATCH":"color:#ff8800;font-weight:bold",
                           "CAUTION":"color:#ffcc00","OK":"color:#00cc66"}
            def style_weak(col):
                if col.name == "Weakness":
                    return [weak_colors.get(str(v), "") for v in col]
                return ["" for _ in col]
            st.dataframe(weak.style.apply(style_weak, axis=0), use_container_width=True, height=600)
